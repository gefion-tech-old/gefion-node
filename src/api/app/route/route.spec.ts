...
describe('RouteService в RouteModule', () => {
    ...
    `, async () => {
        ...
        await expect(metadataRepository.count()).resolves.toBe(metadataEntityCount + 2)
    ...
    it(`
        Попытка установить метаданные связи несуществующего маршрута с контроллером приводит к исключению 
    `, async () => {
        const container = await getContainer()
        container.snapshot()
        ...
        await expect(routeService.setRouteControllerMetadata(route, {
            metadata: {
                custom: null
            },
            revisionNumber: 0
    ...
    it(`
        Метаданные корректно устанавливаются в связь маршрута с контроллером и читаются из неё. Попытка установить 
        метаданные несоответствующей редакции приводит к исключению
    `, async () => {
        const container = await getContainer()
        container.snapshot()
