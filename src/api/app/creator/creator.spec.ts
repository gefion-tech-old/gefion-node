...
describe('CreatorService в CreatorModule', () => {
    ...
    describe('Охранники в качестве ресурсов', () => {

        it(`
            Охранники корректно привязываются к экземпляру блока. Нельзя удалить
            экземпляр блока с привязанным охранником, но можно удалить охранник.
        `, async () => {
            const container = await getContainer()
            container.snapshot()
        ...
        it(`
            Охранники корректно привязываются к системе. Привязка удаляется вместе
            с удалением охранника
        `, async () => {
            const container = await getContainer()
            container.snapshot()
        ...
        it(`
            Экземпляр блока, к которому привязан охранник корректно можно получить
        `, async () => {
            const container = await getContainer()
            container.snapshot()
        ...
        it(`
            Систему, к которой привязан охранник корректно можно получить 
        `, async () => {
            const container = await getContainer()
            container.snapshot()
